  ...
  return (
    <section className="py-20 px-10 border-t border-border bg-zinc-950">
      <div className="max-w-6xl mx-auto">
        <h2 className="mono text-[10px] text-muted uppercase mb-12 tracking-widest">Mental Models & Principles</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {PRINCIPLES.map((item, i) => (
            ...
            >
              <h3 className="text-xl mb-4 font-display uppercase tracking-tight text-secondary group-hover:text-accent transition-colors">{item.title}</h3>
              <p className="text-muted/80 text-sm leading-relaxed font-normal">{item.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
